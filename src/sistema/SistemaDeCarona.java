...
public class SistemaDeCarona {
		...
		if (sessao.getLogin().equals(carona.getDonoDaCarona().getLogin())) { // verifica
			...
			for (Sugestao sugestao : carona.getSugestoes()) {
				if (sugestao.getIdSugestao().equals(idSugestao)) { // verifica
					...
					Resposta resp = new Resposta(idResposta, pontos); // cria
																		// uma
																		// resposta
					idResposta = resp.getIdResposta();
					sugestao.addResposta(sugestao, resp); // adiciona em um mapa
															// a sugestao e a
															// resposta dessa
															// sugestao
				}
	...
	public String solicitarVagaPontoEncontro(String idSessao, String idCarona,
		...
		for (Sugestao sugestao : carona.getSugestoes()) {
			for (Sugestao sugestao2 : sugestao.mapaDeResposta.keySet()) {
				if (sugestao.mapaDeResposta.get(sugestao2).getPontos()
						.contains(ponto)) { // verifica no mapa de sugestoes e
											// respostas se alguma resposta
											// contem o ponto desejado para a
											// carona
	...
	public boolean taNaCarona(String loginCaroneiro, String idCarona) {
		Carona carona = buscaCaronaID(idCarona);
	...
	public void reviewVagaEmCarona(String idSessao, String idCarona,
		...
		if (taNaCarona(loginCaroneiro, idCarona)) {// Se tiver...
	...
	public void reviewCarona(String idSessao, String idCarona, String review)
			throws Exception {
		Carona carona = buscaCaronaID(idCarona);
		Perfil perfil = buscaPerfil(carona.getDonoDaCarona().getLogin());
		Sessao sessao = buscarSessaoId(idSessao);

		if (taNaCarona(sessao.getLogin(), idCarona)) {

			if (review.equals("segura e tranquila")) {
				int segura = perfil.getCaronasSeguras() + 1;
				perfil.setCaronasSeguras(segura);
			} else if (review.equals("não funcionou")) {
				int nFuncionou = perfil.getCaronasNaoFuncionaram() + 1;
				perfil.setCaronasNaoFuncionaram(nFuncionou);
			} else {
				throw new Exception("Opção inválida.");
			}
		} else {
	...
	public static void main(String[] args) throws Exception {
		...
		String idCarona4 = sistema.cadastrarCarona(idSessaoMark, "origemM",
				"destinoM", "12/05/2012", "12:00", "3");
		String idCarona5 = sistema.cadastrarCarona(idSessaoMark, "origemM2",
				"destinoM2", "12/06/2012", "12:00", "3");
		...
		// System.out.println(sistema.getAtributoPerfil("bill",
		// "faltas em vagas de caronas"));
		System.out.println(sistema.getAtributoPerfil("mark",
				"caronas seguras e tranquilas"));
		sistema.reviewCarona(idSessaoBill, idCarona4,
				"caronas seguras e tranquilas");
		System.out.println(sistema.getAtributoPerfil("mark",
				"caronas seguras e tranquilas"));
	}
