...
public class SistemaDeCarona {
	...
	public String responderSugestaoPontoEncontro(String idSessao,
		...
		if (sessao.getLogin().equals(carona.getDonoDaCarona().getLogin())) {
			for (Sugestao sugestao : carona.getSugestoes()) {
				 ...
				 * carona.getSugestoes().clear(); carona.addSugestao(new
				 * Sugestao(pontos, idSessao)); }
				 */
				/*
				 * Agora Sugestao tem um mapa de resposta que a chave é o
				 * IdResposta e a resposta é os pontos
				 */

				if (sugestao.getIdSugestao().equals(idSugestao)) {
					idResposta = UUID.randomUUID().toString();
					sugestao.addResposta(idResposta, pontos);
				}
	...
	public String solicitarVagaPontoEncontro(String idSessao, String idCarona,
		 ...
		 * carona.addSolicitacao(solicitacao); break; }
		 */
	...
	/*
	 * expect "Campina Grande" getAtributoSolicitacao
	 * idSolicitacao=${solicitacao1ID} atributo="origem" expect "João Pessoa"
	 * getAtributoSolicitacao idSolicitacao=${solicitacao1ID} atributo="destino"
	 * expect "Mark Zuckerberg" getAtributoSolicitacao
	 * idSolicitacao=${solicitacao1ID} atributo="Dono da carona" expect
	 * "William Henry Gates III" getAtributoSolicitacao
	 * idSolicitacao=${solicitacao1ID} atributo="Dono da solicitacao" expect
	 * "Acude Velho" getAtributoSolicitacao idSolicitacao=${solicitacao1ID}
	 * atributo="Ponto de Encontro"
	 */
	...
	public void aceitarSolicitacaoPontoEncontro(String idSessao,
		...
		Sessao sessao = buscarSessaoId(idSessao);
		Carona carona = buscaCaronaID(solicitacao.getIdCarona());
		if (buscaUsuario(sessao.getLogin()).getLogin().equals(
				carona.getDonoDaCarona().getLogin())) {
	...
	// desistirRequisicao idSessao=${sessaoMark} idCarona=${carona4ID}
	// idSugestao=${sugestaoID}
	...
	public void desistirRequisicao(String idSessao, String idCarona,
		...
		System.out.println("id Sugestao " + sugestao);
		if (sessao.getLogin().equals(carona.getDonoDaCarona().getLogin())) {
			if (sugestao != null) {
	...
	public static void main(String[] args) throws Exception {
		...
		sistema.criarUsuario("danilo", "daniloSenha", "Danilao",
				"rua do danilo", "Email@");
		sistema.criarUsuario("ght1", "senhaght", "Ght Marques", "Rua do Ght",
				"ght@emai");

		String idSessaoDanilo = sistema.abrirSessao("danilo", "daniloSenha");
		String idCarona = sistema.cadastrarCarona(idSessaoDanilo, "origem1",
				"destino1", "12/04/2013", "12:00", "3");

		sistema.encerrarSessao("danilo");
		String idGht = sistema.abrirSessao("ght1", "senhaght");
		// ADD SUGESTAO

		String idSugestao = sistema.sugerirPontoEncontro(idGht, idCarona,
				"acude Vei;praça");
		// System.out.println(sistema.buscaSugestao(idSugestao, idCarona)
		// +"<----");
		...
		// System.out.println(sistema.buscaSugestao(idSugestao, idCarona)
		// +"<----");
		sistema.responderSugestaoPontoEncontro(idSessaoDanilo2, idCarona,
				idSugestao, "acude Vei");
		// System.out.println(carona.getSugestoes().toString());
		...
		/*
		 * String idGht2 = sistema.abrirSessao("ght1", "senhaght"); String
		 * idSolicitacao = sistema.solicitarVagaPontoEncontro(idGht2, idCarona,
		 * "Parque da crianca");
		 */
